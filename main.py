            ...
            for other in board['snakes']:
                ...
                if abs(enemy_head['x'] - new_x) + abs(enemy_head['y'] - new_y) == 1:
                    if my_length <= len(other['body']):
                        break  # Head-on-Head-Risiko → Move blocken
            else:
                safe_moves.append(m)  # nur wenn kein break ausgelöst wurde
